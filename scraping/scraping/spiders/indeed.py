...
import scraping.scraping.base_spider
import scraping.scraping.items


class IndeedSpider(scraping.scraping.base_spider.BaseSpider):
    name = 'indeed'

    def __init__(self, iso_run_date):
        super(self.__class__, self).__init__(
            ['indeed.com'], scraping.scraping.items.JobItem,
            'https://www.indeed.com', iso_run_date)
    ...
    def parse_job_listings(self, response):
        ...
        if job_ids:
            yield scrapy.Request(
                self.get_abs_url(u'rpc/jobdescs?jks={}'.format(
                    u','.join(job_ids))),
